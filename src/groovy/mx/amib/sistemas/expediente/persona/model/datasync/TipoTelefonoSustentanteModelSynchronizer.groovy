...
class TipoTelefonoSustentanteModelSynchronizer implements IModelSynchronizer {
	...
	public void synchronize(boolean removeMissing) {
		...
		if(removeMissing){
			...
			def ids = TipoTelefonoSustentante.executeQuery("select tps.id from TipoTelefonoSustentante tps where tps.numeroVersion <= :numeroVersion",[numeroVersion:localVersion])
			if(ids.size() > 0){
				List<Long> idsToDelete = this.getIdsToRemove(ids)
				if(idsToDelete != null && idsToDelete.size() > 0)
					TipoTelefonoSustentante.executeUpdate("delete from TipoTelefonoSustentante tps where tps.id in :ids",[ids:idsToDelete])
			}
		}
		
		//actualiza el numero de versión
		def vc = VersionCatalogo.findByNumeroCatalogo(105)
	...
	@Override
	public void refresh() {
		TipoTelefonoSustentante.executeUpdate("delete from TipoTelefonoSustentante")
	...
	public long checkLocalVersion() {
		Long numero = VersionCatalogo.findByNumeroCatalogo(105).numeroVersion
		return numero.value
	}
	...
	public long checkRemoteVersion() {
		long result = -1
		Long numeroAmibCatalogos = VersionCatalogo.findByNumeroCatalogo(105).numeroCatalogoForaneo
	...
	public Collection downloadAllElements() {
		List<TipoTelefonoSustentante> allElements = new ArrayList<TipoTelefonoSustentante>()
		...
		resp.json.each{
			TipoTelefonoSustentante x = new TipoTelefonoSustentante()
			x.id = it.id
	...
	public Collection downloadUpdatedElements(long version) {
		List<TipoTelefonoSustentante> updatedElements = new ArrayList<TipoTelefonoSustentante>()
